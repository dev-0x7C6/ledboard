...
public:
	ws2812b(pwm_type &pwm) : m_pwm(pwm) {}
	
	template <ws_type timings = ws_type::ws2812b>
	void write(u8 *begin, int size) {
		...
		constexpr auto reset_phase = ns::to_cycles(get_ws_timings(timings).reset_phase());
		__builtin_avr_delay_cycles(reset_phase);
	}

private:
	template <ws_type timings>
	void write(bool value) {
		constexpr auto hi_phase = get_ws_timings(timings).hi_phase();
		constexpr auto lo_phase = get_ws_timings(timings).lo_phase();
